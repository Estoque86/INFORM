  ...
  BOOST_FOREACH (const Link &link, m_linksList)
    {
      ...
      uint32_t idFrom = link.GetFromNode()->GetId();
      uint32_t idTo = link.GetToNode()->GetId();
      NS_LOG_UNCOND("FROM NODE\t" << idFrom << "\tTO NODE\t" << idTo << "\tDELAY\t" << metricDelay);
      ...
      uint32_t numInterfFrom = link.GetFromNode()->GetNDevices();
      uint32_t numInterfTo = link.GetToNode()->GetNDevices();

      NS_LOG_UNCOND("FROM NODE\t" << idFrom << "\tWITH\t" << numInterfFrom << "\tINTERFACES- TO NODE\t" << idTo << "\tWITH\t" << numInterfTo << "\tINTERFACES");

      {
        ...
        uint32_t numFacesFrom = ndn->GetNFaces();

                NS_LOG_UNCOND("FROM NODE\t" << idFrom << "\tWITH\t" << numFacesFrom << "\tFACES");

	if (ndn != 0)
          {
            Ptr<ndn::Face> face = ndn->GetFaceByNetDevice (link.GetFromNetDevice ());

            uint32_t idFace = face->GetId();
            NS_LOG_UNCOND("FROM NODE\t" << idFrom << "\tFACE\t" << idFace);
