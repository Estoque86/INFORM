...
{
  uint32_t size = 2 + ((2 + 2) + (4) + (4) + (m_name->GetSerializedSize ()) + (2 + 2 + 4 + 2 + 2 + (2 + 0)));
  if (m_signature != 0)
    size += 4;
...
ContentObject::Serialize (Buffer::Iterator start) const
{
  ...
  // Additional Info - qValue
  if (!GetAdditionalInfo ().GetQvalue ().IsStrictlyNegative())
  {
	  start.WriteU32 (static_cast<uint32_t> (GetAdditionalInfo ().GetQvalue().ToInteger(Time::NS)));
  }
...
ContentObject::Deserialize (Buffer::Iterator start)
{
  ...
  GetAdditionalInfo ().SetQvalue(NanoSeconds(i.ReadU32 ()));
...
void
ContentObject::AdditionalInfo::SetQvalue (const Time qValue)
{
  m_qValue = qValue;
}

Time
ContentObjectHeader::AdditionalInfo::GetQvalue () const
{
  return m_qValue;
}
